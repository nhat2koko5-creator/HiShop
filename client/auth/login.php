...
// Khởi tạo mảng $errors (thay vì $login_errors)
$errors = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        try {
            ...
            // (Sử dụng tên cột từ CSDL hishop_db.sql của bạn)
            $stmt = $pdo->prepare("SELECT * FROM nguoi_dung WHERE email = ?");
            $stmt->execute([$email]);
            $user = $stmt->fetch();

            // 6. KIỂM TRA NGƯỜI DÙNG VÀ MẬT KHẨU
            // $user = true NẾU TÌM THẤY
            // password_verify = true NẾU MẬT KHẨU KHỚP
            if ($user && password_verify($mat_khau, $user['mat_khau'])) {
                ...
                // 8. Chuyển hướng người dùng về trang chủ
                header("Location: index.php?page=home");
                exit;

            } else {
                // 9. ĐĂNG NHẬP THẤT BẠI
                // (Dùng thông báo chung để bảo mật)
    ...
    <div class="auth-card">
        ...
        <?php if (isset($_GET['register']) && $_GET['register'] == 'success'): ?>
            <div class="error-message" style="background-color: #D1FAE5; color: #065F46; border-color: #6EE7B7;">
                <p>✅ Đăng ký tài khoản thành công! Vui lòng đăng nhập để tiếp tục.</p>
            </div>
        <?php endif; ?>
