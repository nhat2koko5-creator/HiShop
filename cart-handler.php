    ...
    case 'update':
        $item_id = (int)$cart_key; // đây có thể là bien_the_id hoặc san_pham_id
        if ($item_id <= 0 || $quantity < 1) {
            $response['message'] = 'Lỗi cập nhật.';
            break;
        }

        // Kiểm tra xem bảng có cột bien_the_id không
        $stmtCols = $pdo->prepare("SHOW COLUMNS FROM gio_hang LIKE 'bien_the_id'");
        $stmtCols->execute();
        $hasBienTheColumn = (bool)$stmtCols->fetch();

        if ($hasBienTheColumn) {
            // UPDATE THEO BIẾN THỂ
            $sql = "UPDATE gio_hang 
                    SET so_luong = ? 
                    WHERE nguoi_dung_id = ? AND bien_the_id = ?";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([$quantity, $user_id, $item_id]);

            // Lấy giá biến thể
            $stmt_price = $pdo->prepare("SELECT gia FROM bien_the_san_pham WHERE id = ?");
            $stmt_price->execute([$item_id]);
        } else {
            // FALLBACK — Không có biến thể, update theo sản phẩm
            $sql = "UPDATE gio_hang 
                    SET so_luong = ? 
        ...
        $price = (float)$stmt_price->fetchColumn();
    ...
    case 'delete':
        ...
        if ($hasBienTheColumn) {
            $sql = "DELETE FROM gio_hang WHERE nguoi_dung_id = ? AND bien_the_id = ?";
        } else {
